	...
	public static void main(String[] args) {
		...
		while(true) {
			...
			if(command == 1) {
				...
				// update - updat2
				service.memberUpdate(command, null);;
			}else if(command == 4) {
				// delete - delete
				service.memberDelete(command);;
				
			}else if(command == 5) {
				// search - name 1 : MemberDTO
				MemberDTO member = service.getNameMember(null);
				if(member == null) {
				...
				List<MemberDTO> members = service.getNameMembers(null);
				if(members.size() == 0) {
					System.out.println("Member Not Found");					
				}else {
					MemberView.memberPrint(members);
